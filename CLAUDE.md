...
- **react-markdown** for Markdown rendering, via the shared **`components/Prose.tsx`** wrapper used by BOTH the admin editor preview and the public blog post body (keep them in lockstep — edit Prose, not the call sites). Plugins: **remark-gfm** (tables etc.), **rehype-raw** (lets trusted author HTML through — e.g. the `<u>` underline the editor shortcut inserts; safe because it's a single-admin CMS), **rehype-highlight** (tokenizes fenced code into `.hljs-*` spans). Prose styling via `@tailwindcss/typography` (`@plugin` in `styles.css`); `Prose` takes a `className` for size variants (the editor passes `prose-sm`). Fenced code blocks render as a **terminal window** (`TerminalPre` in Prose: ink panel, moss/amber/rust traffic lights, `›_ lang` prompt chip, `not-prose`); `.hljs-*` token colors (gruvbox-bright) live in `styles.css`.
...
src/
  ...
  components/
    ...
    Prose.tsx           # shared react-markdown renderer (editor preview + blog post); remark-gfm + rehype-raw + rehype-highlight; terminal-window code blocks
...
- **Blog post** (`routes/blog/$slug.tsx`): public read page — fetches `GET /blogs/:slug` (which counts a view), renders banner + tags + date + **markdown body** (via the shared `<Prose>` — syntax-highlighted terminal code blocks), and a footer with view count + `LikeButton`. Loading + 404 states.
- **Admin panel** (`routes/admin/*`) — **fully wired to the backend (real auth + live data).** Content width is `max-w-6xl` (wider than the public `max-w-5xl`). The public `Navbar` is hidden for `/admin` paths via a `pathname.startsWith('/admin')` check in `__root.tsx`. `admin/route.tsx` wraps the subtree in `AuthProvider` and **gates on auth**: while `/auth/me` resolves → spinner; guest → `AdminLogin`; authed → the panel's **own sticky top bar** (pulsing moss status dot, `Admin / Console` brand, Dashboard/New Post tabs, "View site" exit, **Logout** button) + `<Outlet/>`. (UI gating only — real security is the backend `requireAuth` on write routes.)
  ...
  - **Editor** (`admin/editor/index.tsx`): **create OR edit**. `?edit=<id>` (route `validateSearch`) loads the post via `GET /blogs/admin/:id` and pre-fills the fields. Four fields — **Title**, **Banner Image** (URL field OR drag-drop/click file; the actual `File` is tracked in `bannerFile` for multipart upload, preview via `URL.createObjectURL`), **Tags** (chip input), **Content** (Markdown `<textarea>` + live `<Prose>` preview; side-by-side on `lg`, Write/Preview toggle on mobile). The textarea has **`⌘/Ctrl+B / I / U`** shortcuts (and a matching B/I/U toolbar) that wrap the selection with `**`/`*`/`<u></u>` via `wrapSelection` (toggles off if already wrapped; reselects the inner text). `contentRef` is the textarea ref. Save-draft / Publish submit a **`FormData`** to `POST /blogs` (create) or `PUT /blogs/:id` (edit) with `headers: {}` so the browser sets the multipart boundary; banner sent as a file (`banner`) or `bannerImg` URL, tags as a JSON string. Submitting state + spinner, error banner from the API, redirect to the dashboard on success.
